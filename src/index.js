...
(async () => {
	...
	let prNumber = getInput('pr-number');
	let pr;

	if (prNumber) {
		pr = await getPrObject({
			token: GITHUB_TOKEN,
	...
	assert(pr, 'Input "pr" undefined set. Required for accessing and reporting on the PR.');
	...
	if (sizeReport) {
		log.startGroup('Adding summary & comment');

		// await summary.addRaw(sizeReport).write();
		await summary
		...
		try {
			...
			log.error(error);
			log.error('Failed to update comment. Please check the error above & use summary instead.');
		} finally {
			log.endGroup();
		}
	}
