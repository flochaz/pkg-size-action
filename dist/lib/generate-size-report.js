		...
		var HttpClient = class {
			...
			requestRawWithCallback(info2, data, onResult) {
				...
				req.setTimeout(this._socketTimeout || 3 * 6e4, () => {
					...
					handleResult(
						new Error("Request timeout: " + info2.options.path),
						null
					);
				});
			...
			_mergeHeaders(headers) {
				const lowercaseKeys = (obj) =>
					Object.keys(obj).reduce(
						(c2, k) => ((c2[k.toLowerCase()] = obj[k]), c2),
						{}
					);
		...
		(function (global2, factory) {
			...
			class ByteSize {
				...
				toString() {
					...
					return options.toStringFn
						? options.toStringFn.bind(this)()
			...
			function outdent3(stringsOrOptions) {
				...
				if (isTemplateStringsArray(stringsOrOptions)) {
					...
					var firstInterpolatedValueSetsIndentationLevel =
						(values[0] === outdent3 || values[0] === defaultOutdent) &&
...
__export(exports, {
	default: () => generate_size_report_default,
});
...
function createRound(methodName) {
	...
	return function (number, precision) {
		...
		precision =
			precision == null ? 0 : nativeMin(toInteger_default(precision), 292);
...
function property(path2) {
	return isKey_default(path2)
		? baseProperty_default(toKey_default(path2))
		: basePropertyDeep_default(path2);
}
...
function createAggregator(setter, initializer) {
	return function (collection, iteratee) {
		...
		return func(
			collection,
			setter,
			baseIteratee_default(iteratee, 2),
			accumulator
		);
	};
}
...
function partionHidden(hideFilesGlob, files) {
	...
	const hideFilesPtrn = (0, import_glob_to_regexp.default)(hideFilesGlob, {
		extended: true,
	});
...
function parseDisplaySize(displaySize) {
	return displaySize
		.split(",")
		.map((s) => s.trim())
		.filter((s) => supportedSizes.hasOwnProperty(s))
		.map((s) => supportedSizes[s]);
}
var listSizes = (displaySizes, callback) =>
	displaySizes
		.map(({ property: property2 }) => callback(property2))
		.join(" / ");
...
function comparePackages(head, base, { sortBy, sortOrder, hideFiles } = {}) {
	const fileMap = {};
	...
	const [unchanged, changed] = partition_default(
		files,
...
var formatDelta = ({ delta, percent: percent2 }) =>
	delta ? percent2 + directionSymbol(delta) : "";
function generateComment({
	...
	(0, import_core.setOutput)("regressionData", regressionData);
	...
	const table = (0, import_markdown_table.default)(
		[
			...
			...[...changed, ...(unchangedFiles === "show" ? unchanged : [])].map(
				(file) => [
					file.label,
					file.base && file.base.size
						? listSizes(displaySizes, (p) =>
								c((0, import_byte_size.default)(file.base[p]))
						  )
						: "\u2014",
					file.head && file.head.size
						? listSizes(
								displaySizes,
								(p) =>
									(file.base && file.base[p]
										? sup(formatDelta(file.diff[p]))
										: "") + c((0, import_byte_size.default)(file.head[p]))
						  )
						: "\u2014",
				]
			),
			[
				`${strong("Total")} ${
					unchangedFiles === "show" ? "" : sub("_(Includes all files)_")
				}`,
				listSizes(displaySizes, (p) =>
					c((0, import_byte_size.default)(regressionData.base[p]))
				),
				listSizes(
					displaySizes,
					(p) =>
						sup(formatDelta(regressionData.diff[p])) +
						c((0, import_byte_size.default)(regressionData.head[p]))
				),
			],
			[
				strong("Tarball size"),
				...
				sup(formatDelta(regressionData.diff.tarballSize)) +
					c((0, import_byte_size.default)(regressionData.head.tarballSize)),
			],
		],
		{
			align: ["", "r", "r"],
...
${hiddenTable}
	...
	### 📊 Package size report&nbsp;&nbsp;&nbsp;<kbd>${
		formatDelta(regressionData.diff.size) || "No changes"
	}</kbd>
...
var import_outdent2 = __toModule(require_lib());
function headOnly({ headPkgData, hideFiles, displaySize, sortBy, sortOrder }) {
	const displaySizes = parseDisplaySize(displaySize);
	const sizeHeadingLabel = getSizeLabels(displaySizes);
	...
	const table = (0, import_markdown_table2.default)(
		[
			...
			...files.map((file) => [
				file.label,
				listSizes(displaySizes, (p) =>
					c((0, import_byte_size2.default)(file[p]))
				),
			]),
			[
				strong("Total"),
				listSizes(displaySizes, (p) =>
					c((0, import_byte_size2.default)(headPkgData[p]))
				),
			],
			[
				strong("Tarball size"),
...
async function exec(command, options) {
	...
	let exitCode = "127";
	try {
		exitCode = await (0, import_exec.exec)(
			command,
			null,
...
async function isBaseDiffFromHead(baseRef) {
	...
	const { exitCode } = await exec_default(
		`git diff --quiet origin/${baseRef}`,
...
async function npmCi({ cwd } = {}) {
	if (import_fs.default.existsSync("node_modules")) {
		import_core2.info("Cleaning node_modules");
	...
	const { exitCode, stdout, stderr } = await exec_default(
		installCommand,
		options
	);
	if (exitCode > 0) {
...
async function buildRef({
	...
	import_core2.info(`Current working directory: ${cwd}`);
	if (checkoutRef) {
		import_core2.info(`Checking out ref '${checkoutRef}'`);
...
${error.message}`);
			...
			const { exitCode, duration, stdout, stderr } = await exec_default(
				buildCommand,
				{ cwd }
			).catch((error) => {
